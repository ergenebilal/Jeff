...
_worker_loops = []         # keep worker loops alive after worker thread exit


def _close_persistent_tool_loops():
    """Close long-lived bridge loops at interpreter shutdown."""
    loops = []
    if _tool_loop is not None:
        loops.append(_tool_loop)
    loops.extend(_worker_loops)
    for loop in loops:
        try:
...
atexit.register(_close_persistent_tool_loops)
    ...
    if loop and loop.is_running():
        ...
        try:
            ...
            if loop_ready.wait(timeout=1.0) and worker_loop is not None:
                ...
                # The executor may time out before the worker ever starts
                # consuming the coroutine (the regression test uses a fake
                # executor with exactly that behavior).  Close it here so
                # -W error does not surface an unawaited coroutine warning.
                try:
                    coro.close()
                except Exception:
                    pass
