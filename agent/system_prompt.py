...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # Hermes continuous-learning context: conversation-start lessons are
    # volatile and best-effort, so a learning-store issue must never block
    # prompt construction.
    try:
        from brain.context_loader import get_relevant_context

        _learning_context = get_relevant_context()
        if _learning_context:
            volatile_parts.append("Hermes learned context:\n" + _learning_context)
    except Exception:
        pass
