...
def isolated_home():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        os.environ["HOME"] = tmp
        for module_name in ["brain.learning", "brain.context_loader"]:
...
def test_save_lesson_clamps_importance_and_appends_jsonl(isolated_home, monkeypatch):
    ...
    lessons_file = isolated_home / ".hermes" / "learning" / "lessons.jsonl"
    stored = json.loads(lessons_file.read_text(encoding="utf-8").strip())
    assert stored["lesson"] == "Kisa ozet once gelir"
    assert stored["source"] == "test"


def test_recent_lessons_are_newest_first_and_filter_by_category(monkeypatch):
    learning = importlib.import_module("brain.learning")
    monkeypatch.setattr(learning, "_push_to_mnemosyne", lambda entry: True)
...
def test_lessons_summary_is_conversation_ready(monkeypatch):
    learning = importlib.import_module("brain.learning")
    monkeypatch.setattr(learning, "_push_to_mnemosyne", lambda entry: True)
...
def test_mark_applied_increments_matching_trigger(monkeypatch):
    learning = importlib.import_module("brain.learning")
    pushed = []
    monkeypatch.setattr(learning, "_push_to_mnemosyne", lambda entry: pushed.append(entry) or True)
    ...
    assert pushed[-1]["applied_count"] == 1


def test_sync_all_to_mnemosyne_returns_count(monkeypatch):
    learning = importlib.import_module("brain.learning")
    monkeypatch.setattr(learning, "_push_to_mnemosyne", lambda entry: True)
...
def test_context_loader_includes_lessons_time_and_reasoning(monkeypatch):
    learning = importlib.import_module("brain.learning")
    monkeypatch.setattr(learning, "_push_to_mnemosyne", lambda entry: True)
